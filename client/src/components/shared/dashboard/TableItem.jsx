...
const TableItem = ({ table, expanded, toggleExpand }) => {
  ...
  return (
    <div key={table.id}>
      {/* Riga principale */}
      <div
        className={`grid grid-cols-5 align-item items-center py-2 min-w-[972px] cursor-pointer ${table.status === "Pagato" ? "bg-[#EBF9EE]" : "bg-[#F9F9F9]"}`}
      ...
      {expanded === table.id && table.items.length > 0 && (
        <div className="min-w-[972px] px-8 py-2 text-sm text-gray-700 bg-gray-50">
          <ul className="mb-2">
